...
var $author$project$Main$UserModel = function (a) {
	return {$: 'UserModel', a: a};
};
...
var $author$project$Session$LoggedInData = F4(
	function (name, token, room, owner) {
		return {name: name, owner: owner, room: room, token: token};
	});
...
var $author$project$Session$decodeUser = $elm$json$Json$Decode$oneOf(
	_List_fromArray(
		[
			A2(
			...
			A5(
				...
				$author$project$Session$LoggedInData,
			...
			$elm$json$Json$Decode$null($author$project$Session$Guest)
		]));
var $elm$json$Json$Decode$decodeValue = _Json_run;
var $author$project$WebData$NotLoaded = {$: 'NotLoaded'};
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Page$User$init = function (session) {
	return _Utils_Tuple2(
		{response: $author$project$WebData$NotLoaded, session: session, username: ''},
...
var $author$project$Main$init = F3(
	function (user, url, key) {
		var parsedUser = function () {
			var _v2 = A2($elm$json$Json$Decode$decodeValue, $author$project$Session$decodeUser, user);
			if (_v2.$ === 'Ok') {
				var val = _v2.a;
				return val;
			} else {
		...
		var _v0 = $author$project$Page$User$init(
			{key: key, url: url, user: parsedUser});
		var pageModel = _v0.a;
		var pageCmd = _v0.b;
		...
		return _Utils_Tuple2(
			{
				...
				pageModel: $author$project$Main$UserModel(pageModel)
			},
...
var $author$project$Session$encodeUser = function (user) {
	if (user.$ === 'Guest') {
		...
		return $elm$json$Json$Encode$object(
			_List_fromArray(
				[
					...
					_Utils_Tuple2(
					'token',
					$elm$json$Json$Encode$string(data.token)),
					_Utils_Tuple2(
					'room',
					A2($author$project$Utils$encodeMaybe, $elm$json$Json$Encode$string, data.room)),
					_Utils_Tuple2(
					'owner',
					$elm$json$Json$Encode$bool(data.owner))
				]));
	}
};
...
var $author$project$Main$storeSession = _Platform_outgoingPort('storeSession', $elm$core$Basics$identity);
var $elm$http$Http$BadBody = function (a) {
	return {$: 'BadBody', a: a};
};
var $author$project$WebData$Error = function (a) {
	return {$: 'Error', a: a};
};
var $author$project$WebData$Loading = {$: 'Loading'};
var $elm$core$Basics$ge = _Utils_ge;
var $elm$core$String$trim = _String_trim;
var $author$project$Page$User$validateUsername = function (username) {
	var trimmed = $elm$core$String$trim(username);
	return ($elm$core$String$length(trimmed) <= 2) ? $elm$core$Maybe$Just('Username too short.') : (($elm$core$String$length(trimmed) >= 40) ? $elm$core$Maybe$Just('Username too long.') : $elm$core$Maybe$Nothing);
};
var $author$project$Page$User$update = F2(
	function (msg, model) {
		if (msg.$ === 'UpdateUsername') {
			var username = msg.a;
			return _Utils_Tuple2(
				_Utils_update(
					model,
			...
			var _v1 = $author$project$Page$User$validateUsername(model.username);
			if (_v1.$ === 'Nothing') {
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{response: $author$project$WebData$Loading}),
					$elm$core$Platform$Cmd$none);
			} else {
				var error = _v1.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							response: $author$project$WebData$Error(
								$elm$http$Http$BadBody(error))
						}),
					$elm$core$Platform$Cmd$none);
			}
		}
	});
var $author$project$Main$updatePage = F4(
	function (mapMsg, mapModel, model, _v0) {
		var pageModel = _v0.a;
		var cmd = _v0.b;
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					pageModel: mapModel(pageModel)
				}),
			A2($elm$core$Platform$Cmd$map, mapMsg, cmd));
	});
			...
			default:
				if (_v0.b.$ === 'UserModel') {
					...
					return A4(
						...
						$author$project$Main$UserModel,
						model,
						A2($author$project$Page$User$update, userMsg, userModel));
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				}
		}
	});
var $author$project$Main$updateWithStore = F2(
	function (msg, model) {
		var _v0 = A2($author$project$Main$update, msg, model);
		var newModel = _v0.a;
		var newCmd = _v0.b;
		var newSession = $author$project$Main$getSession(newModel);
		return _Utils_Tuple2(
			newModel,
			$elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						...
						$author$project$Session$encodeUser(newSession.user))
					])));
	});
		...
		[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$href('https://github.com/DylanDmitri/spamalot/issues'),
...
var $author$project$Main$viewHeader = function (navbarModel) {
	return A2(
		...
		A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$href('https://github.com/DylanDmitri/spamalot')
						]),
					_List_fromArray(
						[
...
var $author$project$Page$User$Submit = {$: 'Submit'};
var $author$project$Page$User$UpdateUsername = function (a) {
...
var $author$project$Page$User$helpText = 'Please use your real name. Your group needs to be able to identify ' + 'you without having to ask who you are.';
...
var $author$project$Page$User$rulesText = ' Your name must be greater than 2 characters and less than 40, and ' + 'cannot contain any special characters other than space. ';
...
var $author$project$Utils$textInput = function (config) {
	return A2(
			...
			[
				...
				A2(
					...
					_List_fromArray(
						[
							$elm$core$Maybe$Just(
							$rundis$elm_bootstrap$Bootstrap$Form$Input$id(config.id)),
							$elm$core$Maybe$Just(
							$rundis$elm_bootstrap$Bootstrap$Form$Input$value(config.value)),
							$elm$core$Maybe$Just(
							$rundis$elm_bootstrap$Bootstrap$Form$Input$onInput(config.onInput)),
							$elm$core$Maybe$Just(
							$rundis$elm_bootstrap$Bootstrap$Form$Input$disabled(config.disabled)),
							A2(
							$elm$core$Maybe$map,
							function (_v0) {
								return $rundis$elm_bootstrap$Bootstrap$Form$Input$danger;
							},
							config.dangerText)
						]))),
				function () {
				var _v1 = config.dangerText;
				if (_v1.$ === 'Just') {
					var danger = _v1.a;
					return A2(
					...
					return A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$text(config.helpText)
							]));
				}
			}()
			]));
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Faster Avalon Roles')
							]))
					])),
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text($author$project$Page$User$helpText)
									])),
								A2(
								...
								_List_fromArray(
									[
										$author$project$Utils$textInput(
										{dangerText: errorText, disabled: loading, helpText: $author$project$Page$User$rulesText, id: 'username', name: 'Your Name', onInput: $author$project$Page$User$UpdateUsername, value: model.username}),
										A2(
										$rundis$elm_bootstrap$Bootstrap$Button$button,
										_List_fromArray(
											[
												$rundis$elm_bootstrap$Bootstrap$Button$primary,
												$rundis$elm_bootstrap$Bootstrap$Button$onClick($author$project$Page$User$Submit),
												$rundis$elm_bootstrap$Bootstrap$Button$disabled(loading)
											]),
										_List_fromArray(
											[
...
var $author$project$Main$viewPage = function (pageModel) {
	if (pageModel.$ === 'Loading') {
		...
		return A2(
			$elm$html$Html$map,
			$author$project$Main$UserMsg,
			$author$project$Page$User$view(model));
	}
};
var $author$project$Main$view = function (model) {
	return _List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Main$viewPage(model.pageModel)
				])),
...
var $author$project$Main$main = $elm$browser$Browser$application(
	{
		...
		update: $author$project$Main$updateWithStore,
		view: function (model) {
			return {
